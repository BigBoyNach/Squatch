...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName("clear")
        .setDescription(
          "Enter the total number of messages you would like to clear, Please note it must be below 100"
        )
        .setRequired(true)
    ),
  async execute(interaction, client) {
    ...
    if (!DeleteTotal)
      return interaction.editReply({ embeds: [MessageDeletion] });

    if (DeleteTotal > 100)
      return interaction.editReply({ embeds: [MessageLimit] });
    ...
    interaction.channel.bulkDelete(DeleteTotal, true).then((Amount) => {
      ...
      return interaction.channel.send({ embeds: [Successfullydeleted] });
    });
  },
};
