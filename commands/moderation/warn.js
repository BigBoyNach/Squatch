...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription("Please mention the user you would like to warn")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("reason")
        .setDescription(
          "Please enter the reason why you would to warn the user"
        )
        .setRequired(true)
    ),
  async execute(interaction, client) {
    ...
    if (moderator.id == toWarn.id)
      return interaction.editReply({ embeds: [cantwarnyourself] });
    if (
